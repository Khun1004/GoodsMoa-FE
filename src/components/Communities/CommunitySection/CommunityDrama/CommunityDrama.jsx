...
const CommunityDrama = () => {
    const [dramaPosts, setDramaPosts] = useState([]);
    ...
    useEffect(() => {
        const savedPosts = JSON.parse(localStorage.getItem('dramaPosts')) || [];
        ...
        setDramaPosts([...originalPosts, ...formattedPosts]);
    }, []);

    useEffect(() => {
        const savedComments = JSON.parse(localStorage.getItem('dramaComments')) || {};
        setComments(savedComments);
    }, []);

    const handlePostClick = (post) => {
        const updatedPosts = dramaPosts.map(p =>
            p.id === post.id ? { ...p, viewCount: (p.viewCount || 0) + 1 } : p
        );
        setDramaPosts(updatedPosts);

        const userPosts = updatedPosts.filter(p =>
            !originalPosts.some(original => original.id === p.id)
        );
        localStorage.setItem('dramaPosts', JSON.stringify(userPosts));

        setSelectedPost({ ...post, viewCount: (post.viewCount || 0) + 1 });
    };

    let displayedPosts = [...dramaPosts];
    if (filter === '인기') {
    ...
    const handleAddComment = () => {
        ...
        localStorage.setItem('dramaComments', JSON.stringify(updatedComments));
        setNewComment('');
    };
    ...
    return (
        <div className="drama-container">
            <div className="drama-header">
                <h2 className="drama-title">드라마 토론 게시판</h2>
                <div className="drama-select-wrapper">
            ...
            {selectedPost && filter !== '목록' && (
                <>
                    <div className="drama-detail">
                        ...
                        <div className="drama-post-meta">
                            <span className="drama-post-author">작성자: {selectedPost.author}</span>
                            <span className="drama-post-date">작성일: {selectedPost.date}</span>
                            <span className="drama-post-views">조회수: {selectedPost.viewCount}</span>
                        </div>
                        <img src={selectedPost.image} alt="drama" className="drama-detail-img" />
                        <p>{selectedPost.content}</p>
                    </div>
                    <div className="drama-comment-form">
                        <input
                            type="text"
                            value={newComment}
                    ...
                    <div className="drama-comments">
                        <h4 className='drama-comments-title'>댓글</h4>
                        <ul>
                            {(comments[selectedPost.id] || []).map((cmt, idx) => (
                                <li className="drama-comment-item" key={idx}>
                                    ...
                                    <div className="drama-reply-form">
                                        <input
                                            type="text"
                                    ...
                                    {cmt.replies &&
                                        cmt.replies.map((reply, ridx) => (
                                            <div key={ridx} className="drama-reply-item">
                                                <img src={reply.profile} alt="profile" className="reply-profile" />
                                                <div>
                ...
                <tbody>
                    {displayedPosts.map((post, index) => (
                        <tr key={post.id}>
                            <td>{index + 1}</td>
                            <td>
                                <span className="drama-link" onClick={() => handlePostClick(post)}>
                                    {post.title}
                                </span>
                            </td>
                            <td>{post.author}</td>
